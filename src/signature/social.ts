...
import { keyFromPrivate, pemToKey, SigAlg, signRSAMessage } from '../utils'
import { calcEthPersonalHash, signSecp256k1Message } from './secp256k1'
...
export const signSocialTx = async (
    ...
    const webAuthnSignData = `0x${authData}${clientDataHash}`
      ...
      friends: socialUnlockReq.friends.map((friend) => {
        if (friend.algIndex == 1) {
            ...
            return {
                ...
                signature: signSecp256r1Message(key, webAuthnSignData),
            ...
            const message = calcEthPersonalHash(hasher.array())

            const key = keyFromPrivate(friend.privateKey, SigAlg.Secp256k1)
            return {
                lockScript: friend.lockScript,
                pubkey: friend.pubkey,
                signature: signSecp256k1Message(key, message),
                unlockMode: friend.unlockMode,
                algIndex: friend.algIndex,
              }
        } else {
            ...
            return {
                lockScript: friend.lockScript,
                pubkey: friend.pubkey,
                signature: signRSAMessage(key, webAuthnSignData),
                unlockMode: friend.unlockMode,
                algIndex: friend.algIndex,
                webAuthnMsg: `0x${authData}${clientData}`
            }
